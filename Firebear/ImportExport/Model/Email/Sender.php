...
class Sender
{
    ...
    {
        $data = $this->getData($job->getSourceData());
        if (!isset($data['email_type'])) {
            $data = $this->getData($job->getExportSource());
        }

        if (null !== $data && $this->isSend($data, $status)) {
            $this->processLogger->info(__('sending email'));
     ...
     * @return array|null
     */
    protected function getData($sourceData)
    {
        if (empty($sourceData)) {
            return null;
        }
        return $this->jsonDecoder->decode($sourceData);
    }
