...
class SearchTerms extends AbstractEntity implements ImportAdapterInterface
{
            ...
            foreach ($bunch as $rowNum => $rowData) {
                if ($this->validateRow($rowData, $rowNum)) {
                    ...
                    if (Import::BEHAVIOR_REPLACE == $this->getBehavior()) {
                        $this->getDataForReplace($rowData);
                    }
                }
            }
     ...
     * @return array
     */
    protected function getDataForReplace(array $rowData)
    {
        $storeId = $rowData[self::COL_STORE_ID] ?? false;
        $queryText = $rowData[self::COL_QUERY_TEXT] ?? false;
        if ($queryText && $storeId && isset($this->searchQueries[$queryText][$storeId])) {
            $this->searchQueriesForReplace[$queryText][$storeId] = 1;
        }

        return $this->searchQueriesForReplace;
    }
            ...
            try {
                if (Import::BEHAVIOR_REPLACE == $this->getBehavior() &&
                    ...
                    foreach ($rowsToInsert as $rowData) {
                        if (!empty($this->searchQueriesForReplace[$rowData[self::COL_QUERY_TEXT]]
                        [$rowData[self::COL_STORE_ID]])
                        ) {
                            $rowsToUpdate[] = $rowData;
                        }
                    }
